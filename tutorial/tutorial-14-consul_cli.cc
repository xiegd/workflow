...
void print_discover_result(
    std::vector<struct protocol::ConsulServiceInstance> &discover_result) {
  for (const auto &instance : discover_result) {
    ...
    fprintf(stderr, "\n");
    const std::map<std::string, std::string> &service_meta =
...
int main(int argc, char *argv[]) {
  ...
  if (0 == strcmp(argv[2], "discover")) {
    task = client.create_discover_task("", "dev-wf_test_service_1", 3,
                                       consul_callback);
    ...
    task =
        client.create_register_task("", "dev-wf_test_service_1",
                                    "wf_test_service_id_2", 3, consul_callback);
    ...
    task = client.create_deregister_task("", "wf_test_service_id_2", 3,
                                         consul_callback);
  } else {
