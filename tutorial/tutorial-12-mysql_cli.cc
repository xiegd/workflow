...
void get_next_cmd(WFMySQLTask *task) {
  ...
  while ((fgets(query, 4096, stdin)) && stop_flag == false) {
    ...
    if (strncmp(query, "quit", len) == 0 || strncmp(query, "exit", len) == 0) {
      fprintf(stderr, "Bye\n");
      return;
    }
    ...
    next_task =
        WFTaskFactory::create_mysql_task(*url, RETRY_MAX, mysql_callback);
...
void mysql_callback(WFMySQLTask *task) {
  ...
  } else if (resp->get_packet_type() ==
             MYSQL_PACKET_OK) // just check origin APIs
  {
...
static void sighandler(int signo) { stop_flag = true; }
...
int main(int argc, char *argv[]) {
  ...
  SeriesWork *series = Workflow::create_series_work(
      task, [&wait_group](const SeriesWork *series) { wait_group.done(); });
