...
static const std::map<std::string, int> qtype_map = {
    ...
    {"SOA", DNS_TYPE_SOA}, {"NS", DNS_TYPE_NS},     {"SRV", DNS_TYPE_SRV},
    {"MX", DNS_TYPE_MX}};
...
void show_result(protocol::DnsResultCursor &cursor) {
  ...
  while (cursor.next(&record)) {
    ...
    case DNS_TYPE_SOA:
      ...
      sprintf(information, "%s %s %u %d %d %d %u", soa->mname, soa->rname,
              soa->serial, soa->refresh, soa->retry, soa->expire, soa->minimum);
      info = information;
      break;
    case DNS_TYPE_SRV:
      srv = (struct dns_record_srv *)(record->rdata);
      sprintf(information, "%u %u %u %s", srv->priority, srv->weight, srv->port,
              srv->target);
      info = information;
      break;
    case DNS_TYPE_MX:
    ...
    printf("%s\t%d\t%s\t%s\t%s\n", record->name, record->ttl,
           dns_class2str(record->rclass), dns_type2str(record->type), info);
  }
  printf("\n");
}
...
void dns_callback(WFDnsTask *task) {
  ...
  printf(";; HEADER opcode:%s status:%s id:%d\n",
         dns_opcode2str(resp->get_opcode()), dns_rcode2str(resp->get_rcode()),
         resp->get_id());
  printf(";; QUERY:%d ANSWER:%d AUTHORITY:%d ADDITIONAL:%d\n",
         resp->get_qdcount(), resp->get_ancount(), resp->get_nscount(),
         resp->get_arcount());
