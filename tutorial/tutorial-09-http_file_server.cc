...
void process(WFHttpTask *server_task, const char *root) {
  ...
  if (fd >= 0) {
    ...
    pread_task =
        WFTaskFactory::create_pread_task(fd, buf, size, 0, pread_callback);
...
int main(int argc, char *argv[]) {
  ...
  auto &&create = [&scheme, port](WFRepeaterTask *) -> SubTask * {
    ...
    *buf = '\0';
    ...
    WFHttpTask *task =
        WFTaskFactory::create_http_task(url, 0, 0, [](WFHttpTask *task) {
  ...
  repeater = WFTaskFactory::create_repeater_task(
      create, [&wg](WFRepeaterTask *) { wg.done(); });
