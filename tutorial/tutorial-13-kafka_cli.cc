  ...
  case Kafka_Fetch:
    ...
    if (!records.empty()) {
      ...
      for (const auto &v : records) {
        ...
        char fn[1024];
        snprintf(fn, 1024, "/tmp/kafka.%s.%d.%llu", v.back()->get_topic(),
                 v.back()->get_partition(), v.back()->get_offset());
    ...
    if (!toppars.empty()) {
      for (const auto &v : toppars) {
        printf("commit\ttopic: %s, partition: %d, \
						offset: %llu, error: %d\n",
               v->get_topic(), v->get_partition(), v->get_offset(),
               v->get_error());
      }
    }
...
int main(int argc, char *argv[]) {
  ...
  if (argv[2][0] == 'p') {
    ...
    if (argc > 3 && argv[3][0] == 'd') {
      ...
      task = client.create_kafka_task(
          "topic=workflow_test1&topic=workflow_test2&api=fetch", 3,
          kafka_callback);
    }
