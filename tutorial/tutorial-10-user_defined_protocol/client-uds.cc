...
using WFTutorialTask =
    WFNetworkTask<protocol::TutorialRequest, protocol::TutorialResponse>;
...
public:
  static WFTutorialTask *create_tutorial_task(const struct sockaddr *addr,
                                              socklen_t addrlen, int retry_max,
                                              tutorial_callback_t callback) {
    using NTF = WFNetworkTaskFactory<TutorialRequest, TutorialResponse>;
    WFTutorialTask *task = NTF::create_client_task(
        TT_TCP, addr, addrlen, retry_max, std::move(callback));
...
int main(int argc, char *argv[]) {
  ...
  auto &&create = [path](WFRepeaterTask *) -> SubTask * {
    ...
    *buf = '\0';
    ...
    WFTutorialTask *task = MyFactory::create_tutorial_task(
        (struct sockaddr *)&sun, sizeof sun, 0, [](WFTutorialTask *task) {
  ...
  Workflow::start_series_work(
      repeater, [&wait_group](const SeriesWork *) { wait_group.done(); });
