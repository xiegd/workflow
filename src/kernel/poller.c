...
static inline int __poller_create_pfd() { return kqueue(); }
...
static inline int __poller_create_timerfd() { return 0; }

static inline int __poller_close_timerfd(int fd) { return 0; }

static inline int __poller_add_timerfd(int fd, poller_t *poller) { return 0; }
...
poller_t *__poller_create(void **nodes_buf,
                          const struct poller_params *params) {
...
int poller_mod(const struct poller_data *data, int timeout, poller_t *poller) {
  ...
  if (orig) {
    if (__poller_mod_fd(data->fd, orig->event, node->event, node, poller) >=
        0) {
      if (orig->in_rbtree)
