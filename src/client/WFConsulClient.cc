...
WFConsulTask::WFConsulTask(const std::string &proxy_url,
                           ...
                           const std::string &service_id, int retry_max,
                           consul_callback_t &&cb)
    : proxy_url(proxy_url), callback(std::move(cb)) {
...
static bool
parse_discover_result(const json_value_t *root,
                      std::vector<struct ConsulServiceInstance> &result);
static bool
parse_list_service_result(const json_value_t *root,
...
bool WFConsulTask::check_task_result(WFHttpTask *task,
...
WFConsulTask *
WFConsulClient::create_discover_task(const std::string &service_namespace,
                                     const std::string &service_name,
                                     int retry_max, consul_callback_t cb) {
  WFConsulTask *task =
      new WFConsulTask(this->proxy_url, service_namespace, service_name, "",
...
WFConsulTask *
WFConsulClient::create_list_service_task(const std::string &service_namespace,
                                         int retry_max, consul_callback_t cb) {
...
WFConsulTask *WFConsulClient::create_register_task(
    ...
    const std::string &service_id, int retry_max, consul_callback_t cb) {
  WFConsulTask *task =
      new WFConsulTask(this->proxy_url, service_namespace, service_name,
...
WFConsulTask *
WFConsulClient::create_deregister_task(const std::string &service_namespace,
...
static bool create_tagged_address(const ConsulAddress &consul_address,
  ...
  const json_value_t *val =
      json_object_append(tagged_obj, name.c_str(), JSON_VALUE_OBJECT);
  if (!val)
    return false;
...
static bool create_health_check(const ConsulConfig &config,
                                json_object_t *obj) {
  const json_value_t *val;
  std::string str;
...
static bool create_register_request(const json_value_t *root,
  ...
  for (const auto &meta_kv : service->meta) {
    if (!json_object_append(meta_obj, meta_kv.first.c_str(), JSON_VALUE_STRING,
