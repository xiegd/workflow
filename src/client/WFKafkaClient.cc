...
public:
  KafkaClientTask(const std::string &query, int retry_max,
                  kafka_callback_t &&callback, WFKafkaClient *client)
  ...
  virtual ~KafkaClientTask() { this->member->decref(); }
...
private:
  ...
  static void kafka_merge_meta_list(KafkaMetaList *dst, KafkaMetaList *src);
...
void KafkaClientTask::kafka_rebalance_callback(__WFKafkaTask *task) {
  ...
  if (task->get_state() == WFT_STATE_SUCCESS) {
    ...
    if (member->heartbeat_status == KAFKA_HEARTBEAT_UNINIT) {
      ...
      kafka_task = __WFKafkaTaskFactory::create_kafka_task(
          member->transport_type, coordinator->get_host(),
          coordinator->get_port(), member->ssl_ctx, "", 0,
...
void KafkaClientTask::kafka_rebalance_proc(KafkaMember *member,
  ...
  task = __WFKafkaTaskFactory::create_kafka_task(
      member->transport_type, coordinator->get_host(), coordinator->get_port(),
      member->ssl_ctx, "", 0, kafka_rebalance_callback);
...
void KafkaClientTask::kafka_timer_callback(WFTimerTask *task) {
  ...
  kafka_task = __WFKafkaTaskFactory::create_kafka_task(
      member->transport_type, coordinator->get_host(), coordinator->get_port(),
      member->ssl_ctx, "", 0, kafka_heartbeat_callback);
...
void KafkaClientTask::kafka_meta_callback(__WFKafkaTask *task) {
  ...
  if (t->state == WFT_STATE_SUCCESS) {
    ...
    kafka_merge_broker_list(t->member->scheme, &t->member->broker_hosts,
                            &t->member->broker_map,
...
void KafkaClientTask::kafka_cgroup_callback(__WFKafkaTask *task) {
  ...
  if (t->state == WFT_STATE_SUCCESS) {
    ...
    kafka_merge_broker_list(member->scheme, &member->broker_hosts,
                            &member->broker_map,
                            task->get_resp()->get_broker_list());
...
void KafkaClientTask::kafka_parallel_callback(const ParallelWork *pwork) {
  ...
  for (size_t i = 0; i < pwork->size(); i++) {
    state_error =
        (std::pair<int32_t, int32_t> *)pwork->series_at(i)->get_context();
...
void KafkaClientTask::kafka_move_task_callback(__WFKafkaTask *task) {
  ...
  state_error->second =
      *static_cast<ComplexKafkaTask *>(task)->get_mutable_ctx();
...
void KafkaClientTask::parse_query() {
  ...
  for (const auto &kv : query_kv) {
    if (strcasecmp(kv.first.c_str(), "api") == 0 && api_type == Kafka_Unknown) {
...
bool KafkaClientTask::check_meta() {
  ...
  if (member->meta_doing) {
    ...
    task = __WFKafkaTaskFactory::create_kafka_task(
        this->url, member->ssl_ctx, this->retry_max, kafka_meta_callback);
    task->user_data = this;
    task->get_req()->set_config(this->config);
...
int KafkaClientTask::dispatch_locked() {
  ...
  case Kafka_Produce:
    ...
    for (auto &v : this->toppar_list_map) {
      ...
      task = __WFKafkaTaskFactory::create_kafka_task(
          member->transport_type, broker->get_host(), broker->get_port(),
          member->ssl_ctx, this->get_userinfo(), this->retry_max,
          std::move(cb));
      task->get_req()->set_config(this->config);
  ...
  case Kafka_OffsetCommit:
    ...
    task = __WFKafkaTaskFactory::create_kafka_task(
        member->transport_type, coordinator->get_host(),
        coordinator->get_port(), member->ssl_ctx, this->get_userinfo(),
        this->retry_max, kafka_offsetcommit_callback);
    task->user_data = this;
    task->get_req()->set_config(this->config);
    task->get_req()->set_cgroup(member->cgroup);
  ...
  case Kafka_LeaveGroup:
    ...
    task = __WFKafkaTaskFactory::create_kafka_task(
        member->transport_type, coordinator->get_host(),
        coordinator->get_port(), member->ssl_ctx, this->get_userinfo(), 0,
...
bool KafkaClientTask::add_toppar(const KafkaToppar &toppar) {
  ...
  if (!flag) {
    ...
    if (!new_toppar.set_topic_partition(toppar.get_topic(),
                                        toppar.get_partition())) {
      this->member->mutex.unlock();
      return false;
    }
...
bool KafkaClientTask::add_produce_record(const std::string &topic,
                                         int partition, KafkaRecord record) {
  if (!add_topic(topic))
    return false;
...
static bool check_replace_toppar(KafkaTopparList *toppar_list,
                                 KafkaToppar *toppar) {
  bool flag = false;
  toppar_list->rewind();
  KafkaToppar *exist;
...
int KafkaClientTask::arrange_fetch() {
  ...
  for (auto &topic : topic_set) {
    if (this->member->cgroup.get_group()) {
      ...
      KafkaToppar *toppar;
      while ((toppar = this->member->cgroup.get_assigned_toppar_next()) !=
             NULL) {
        if (topic.compare(toppar->get_topic()) == 0) {
          ...
          if (this->toppar_list_map.find(node_id) ==
              this->toppar_list_map.end())
            this->toppar_list_map[node_id] = (KafkaTopparList());
    ...
    if (toppar->get_partition() < 0) {
      ...
      while ((record = toppar->get_record_next()) != NULL) {
        ...
        if (this->partitioner) {
          ...
          partition = this->partitioner(toppar->get_topic(), key, key_len,
                                        partition_num);
        } else
          partition = rand() % partition_num;

        KafkaToppar *new_toppar =
            get_toppar(toppar->get_topic(), partition, &this->toppar_list);
        if (!new_toppar) {
          KafkaToppar tmp;
...
WFKafkaTask *WFKafkaClient::create_kafka_task(const std::string &query,
                                              ...
                                              kafka_callback_t cb) {
  WFKafkaTask *task =
      new KafkaClientTask(query, retry_max, std::move(cb), this);
  return task;
}
...
WFKafkaTask *WFKafkaClient::create_leavegroup_task(int retry_max,
                                                   kafka_callback_t cb) {
  WFKafkaTask *task =
      new KafkaClientTask("api=leavegroup", retry_max, std::move(cb), this);
  return task;
}
