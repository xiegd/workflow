...
using kafka_partitioner_t =
    std::function<int(const char *topic_name, const void *key, size_t key_len,
                      int partition_num)>;
...
public:
  ...
  protocol::KafkaResult *get_result() { return &this->result; }

  int get_kafka_error() const { return this->kafka_error; }

  void set_callback(kafka_callback_t cb) { this->callback = std::move(cb); }
...
public:
  ...
  // example: api=commit
  WFKafkaTask *create_kafka_task(const std::string &query, int retry_max,
                                 kafka_callback_t cb);
...
public:
  ...
  WFKafkaTask *create_leavegroup_task(int retry_max, kafka_callback_t callback);
