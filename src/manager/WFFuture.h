...
template <typename RES> class WFFuture {
public:
  WFFuture(std::future<RES> &&fr) : future(std::move(fr)) {}
  ...
  std::future_status
  wait_for(const std::chrono::duration<REP, PERIOD> &time_duration) const;
...
template <typename RES> class WFPromise {
public:
  WFPromise() = default;
...
template <typename RES> void WFFuture<RES>::wait() const {
  if (this->future.wait_for(std::chrono::seconds(0)) !=
...
template <class REP, class PERIOD>
std::future_status WFFuture<RES>::wait_for(
    const std::chrono::duration<REP, PERIOD> &time_duration) const {
...
template <typename RES>
template <class CLOCK, class DURATION>
std::future_status WFFuture<RES>::wait_until(
    const std::chrono::time_point<CLOCK, DURATION> &timeout_time) const {
  std::future_status status = std::future_status::ready;
...
template <> inline void WFFuture<void>::get() {
  this->wait();
  this->future.get();
}

template <> class WFPromise<void> {
public:
  WFPromise() = default;
  WFPromise(const WFPromise &promise) = delete;
  WFPromise(WFPromise &&move) = default;
