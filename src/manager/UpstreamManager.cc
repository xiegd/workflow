...
private:
  __UpstreamManager() : mutex(PTHREAD_MUTEX_INITIALIZER) {}
...
int UpstreamManager::upstream_create_consistent_hash(
    const std::string &name, upstream_route_t consistent_hash) {
  WFNameService *ns = WFGlobal::get_name_service();
  UPSConsistentHashPolicy *policy;

  policy = new UPSConsistentHashPolicy(
      consistent_hash ? std::move(consistent_hash) : __default_consistent_hash);
  if (ns->add_policy(name.c_str(), policy) >= 0) {
    __UpstreamManager::get_instance()->add_upstream_policy(policy);
    return 0;
  }
...
int UpstreamManager::upstream_add_server(const std::string &name,
  ...
  UPSGroupPolicy *policy =
      dynamic_cast<UPSGroupPolicy *>(ns->get_policy(name.c_str()));
...
int UpstreamManager::upstream_delete(const std::string &name) {
  WFNameService *ns = WFGlobal::get_name_service();
  UPSGroupPolicy *policy =
      dynamic_cast<UPSGroupPolicy *>(ns->del_policy(name.c_str()));
...
int UpstreamManager::upstream_replace_server(
