 ...
 * - Do not cost any other network resource, We just simulate in local to choose
 * one target properly.
 ...
 *   2. Consistent-Hash
 *   3. Manual-Select
 * - Additional, we support Main-backup & Group for server and working well in
 any mode.
 *
 * @code{.cc}
        upstream_create_weighted_random("abc.sogou", true);
 //UPSTREAM_WEIGHTED_RANDOM upstream_add_server("abc.sogou",
 "192.168.2.100:8081");       //weight=1, max_fails=200
        upstream_add_server("abc.sogou", "192.168.2.100:9090"); //weight=1,
 max_fails=200 AddressParams params = ADDRESS_PARAMS_DEFAULT; params.weight = 2;
        params.max_fails = 6;
        upstream_add_server("abc.sogou", "www.sogou.com", &params); //weight=2,
 max_fails=6
        ...
        upstream_create_consistent_hash("def.sogou",
                                                                    [](const
 char *path, const char *query, const char *fragment) -> int { return
 somehash(...));
                                                                        });
 //UPSTREAM_CONSISTENT_HASH upstream_create_manual("xyz.sogou",
                                                   [](const char *path,
                                                          const char *query,
                                                          const char *fragment)
 -> int { return select_xxx(...));
                                                   },
                                                   true,
                                                   [](const char *path,
                                                          const char *query,
...
public:
   ...
   * @param[in]  try_another      when first choice is failed, try another one
   * or not
   * @return     success/fail
   * @retval     0                success
   ...
   * @retval     0                success
   * @retval     -1               fail, more info see errno
   * @note       consitent_hash need to return value in 0~(2^31-1)
   * Balance/Monotonicity/Spread/Smoothness
   * @note       if consitent_hash==nullptr, upstream will use std::hash with
   * request uri
   */
   ...
   * when first choose server is already down:
   * - if try_another==false, request will be failed
   * - if try_another==true, upstream will choose from alive-servers by
   * weight-random strategy
   */
   ...
   * @param[in]  name             upstream name
   * @param[in]  select           manual select functional, just tell us
   * main-index.
   * @param[in]  try_another      when first choice is failed, try another one
   * or not
   * @param[in]  consitent_hash   consistent-hash functional
   ...
   * @note
   * when first choose server is already down:
   * - if try_another==false, request will be failed, consistent_hash value will
   * be ignored
   * - if try_another==true, upstream will work with consistent hash mode，if
   * consitent_hash==NULL, upstream will use std::hash with request uri
   * @warning    select functional cannot be nullptr!
   */
  static int upstream_create_manual(const std::string &name,
                                    upstream_route_t select, bool try_another,
                                    upstream_route_t consitent_hash);
...
public:
   ...
   * @param[in]  name             upstream name
   * @param[in]  address          ip OR host OR ip:port OR host:port OR
   * /unix-domain-socket
   * @return     success/fail
   * @retval     0                success
   * @retval     -1               fail, more info see errno
   ...
   * @warning    Same address with exactly same params, still means two
   * different server
   */
  static int upstream_add_server(const std::string &name,
                                 const std::string &address,
   ...
   * @param[in]  name             upstream name
   * @param[in]  address          same as address when add by
   * upstream_add_server
   * @return     success/fail
   * @retval     >=0              success, the amount of being removed server
   * @retval     -1               fail, upstream name not found
   * @warning    If server servers has the same address in this upstream, we
   ...
   * @warning    If server servers has the same address in this upstream, then
   * will appear in the vector multiply times
   */
  static std::vector<std::string>
  upstream_main_address_list(const std::string &name);

public:
  /// @breif for plugin
  static int upstream_disable_server(const std::string &name,
                                     const std::string &address);
  static int upstream_enable_server(const std::string &name,
                                    const std::string &address);

  static int
  upstream_replace_server(const std::string &name, const std::string &address,
                          const struct AddressParams *address_params);
};
