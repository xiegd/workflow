...
public:
  ...
  RouteResultEntry() : request_object(NULL), group(NULL) {
...
static inline int __addr_cmp(const struct addrinfo *x,
                             const struct addrinfo *y) {
  // todo ai_protocol
...
static inline bool __addr_less(const struct addrinfo *x,
                               const struct addrinfo *y) {
...
int RouteManager::get(enum TransportType type, const struct addrinfo *addrinfo,
                      const std::string &other_info,
                      const struct EndpointParams *ep_params,
                      const std::string &hostname, SSL_CTX *ssl_ctx,
                      RouteResult &result) {
  ...
  uint64_t key =
      __generate_key(type, addrinfo, other_info, ep_params, hostname, ssl_ctx);
