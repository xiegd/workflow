 ...
 * @note    Only for WFNetworkTask and only when
 * get_state()==WFT_STATE_TASK_ERROR
 */
enum {
  // COMMON
  ...
  // HTTP
  WFT_ERR_HTTP_BAD_REDIRECT_HEADER =
      2001, ///< Http, 301/302/303/307/308 Location header value is NULL
  WFT_ERR_HTTP_PROXY_CONNECT_FAILED =
      2002, ///< Http, proxy CONNECT return non 200

  // REDIS
  WFT_ERR_REDIS_ACCESS_DENIED = 3001, ///< Redis, invalid password
  WFT_ERR_REDIS_COMMAND_DISALLOWED =
      3002, ///< Redis, command disabled, cannot be "AUTH"/"SELECT"

  // MYSQL
  WFT_ERR_MYSQL_HOST_NOT_ALLOWED = 4001, ///< MySQL
  WFT_ERR_MYSQL_ACCESS_DENIED = 4002,    ///< MySQL, authentication failed
  WFT_ERR_MYSQL_INVALID_CHARACTER_SET =
      4003, ///< MySQL, invalid charset, not found in MySQL-Documentation
  WFT_ERR_MYSQL_COMMAND_DISALLOWED =
      4004, ///< MySQL, sql command disabled, cannot be "USE"/"SET NAMES"/"SET
            ///< CHARSET"/"SET CHARACTER SET"
  WFT_ERR_MYSQL_QUERY_NOT_SET =
      4005, ///< MySQL, query not set sql, maybe forget please check
  WFT_ERR_MYSQL_SSL_NOT_SUPPORTED =
      4006, ///< MySQL, SSL not supported by the server
