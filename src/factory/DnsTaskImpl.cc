...
class ComplexDnsTask
    : public WFComplexClientTask<DnsRequest, DnsResponse,
...
struct addrinfo ComplexDnsTask::hints = {.ai_flags =
...
bool ComplexDnsTask::init_success() {
  ...
  if (!this->route_result_.request_object) {
    ...
    ret = WFGlobal::get_route_manager()->get(type, addr, info_, ep, uri_.host,
                                             ssl_ctx_, route_result_);
    freeaddrinfo(addr);
    if (ret < 0) {
...
WFDnsTask *
WFServerTaskFactory::create_dns_task(CommService *service,
