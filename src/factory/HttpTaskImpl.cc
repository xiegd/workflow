...
public:
  ComplexHttpTask(int redirect_max, int retry_max, http_callback_t &&callback)
      : WFComplexClientTask(retry_max, std::move(callback)),
        redirect_max_(redirect_max), redirect_count_(0) {
...
protected:
  ...
  bool redirect_url(HttpResponse *client_resp, const ParsedURI &uri,
                    ParsedURI &new_uri);
...
void ComplexHttpTask::init_failed() { this->set_empty_request(); }
  ...
  if (this->state == WFT_STATE_SUCCESS) {
    ...
    if (this->need_redirect(uri_, new_uri)) {
      if (uri_.userinfo && strcasecmp(uri_.host, new_uri.host) == 0) {
        ...
        HttpRequest *client_req = this->get_req();
        HttpHeaderCursor cursor(client_req);
        struct HttpMessageHeader header = {.name = "Authorization",
...
public:
  ComplexHttpProxyTask(int redirect_max, int retry_max,
...
bool ComplexHttpProxyTask::finish_once() {
  ...
  if (this->state == WFT_STATE_SUCCESS) {
    ...
    if (this->need_redirect(user_uri_, new_uri)) {
      if (user_uri_.userinfo && strcasecmp(user_uri_.host, new_uri.host) == 0) {
        if (!new_uri.userinfo) {
...
WFHttpTask *WFTaskFactory::create_http_task(const std::string &url,
                                            ...
                                            http_callback_t callback) {
  auto *task =
      new ComplexHttpTask(redirect_max, retry_max, std::move(callback));
  ParsedURI uri;
...
WFHttpTask *WFTaskFactory::create_http_task(const std::string &url,
                                            ...
                                            http_callback_t callback) {
  auto *task =
      new ComplexHttpProxyTask(redirect_max, retry_max, std::move(callback));
