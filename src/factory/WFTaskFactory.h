...
using WFRedisTask =
    WFNetworkTask<protocol::RedisRequest, protocol::RedisResponse>;
using redis_callback_t = std::function<void(WFRedisTask *)>;

using WFMySQLTask =
    WFNetworkTask<protocol::MySQLRequest, protocol::MySQLResponse>;
...
public:
  ...
  static WFHttpTask *create_http_task(const ParsedURI &uri, int redirect_max,
                                      int retry_max, http_callback_t callback);
  ...
  static WFRedisTask *create_redis_task(const std::string &url, int retry_max,
                                        redis_callback_t callback);

  static WFRedisTask *create_redis_task(const ParsedURI &uri, int retry_max,
                                        redis_callback_t callback);

  static WFMySQLTask *create_mysql_task(const std::string &url, int retry_max,
                                        mysql_callback_t callback);

  static WFMySQLTask *create_mysql_task(const ParsedURI &uri, int retry_max,
                                        mysql_callback_t callback);

  static WFDnsTask *create_dns_task(const std::string &url, int retry_max,
                                    dns_callback_t callback);

  static WFDnsTask *create_dns_task(const ParsedURI &uri, int retry_max,
                                    dns_callback_t callback);
...
public:
  ...
  static WFFileVIOTask *create_preadv_task(int fd, const struct iovec *iov,
                                           int iovcnt, off_t offset,
                                           fvio_callback_t callback);

  static WFFileVIOTask *create_pwritev_task(int fd, const struct iovec *iov,
                                            int iovcnt, off_t offset,
                                            fvio_callback_t callback);
...
public:
  static WFFileIOTask *create_pread_task(const std::string &pathname, void *buf,
...
public:
  ...
  static WFGoTask *create_go_task(const std::string &queue_name, FUNC &&func,
                                  ARGS &&...args);
...
public:
  static WFEmptyTask *create_empty_task() { return new WFEmptyTask; }
...
public:
  static T *create_client_task(enum TransportType type, const std::string &host,
                               unsigned short port, int retry_max,
...
template <class INPUT, class OUTPUT> class WFThreadTaskFactory {
private:
