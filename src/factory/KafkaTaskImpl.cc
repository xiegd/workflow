...
class __ComplexKafkaTask
    : public WFComplexClientTask<KafkaRequest, KafkaResponse, int> {
public:
...
CommMessageOut *__ComplexKafkaTask::message_out() {
  ...
  if (seqid == 0) {
    ...
    this->get_connection()->set_context(
        conn_info, [](void *ctx) { delete (KafkaConnectionInfo *)ctx; });
...
bool __ComplexKafkaTask::init_success() {
  ...
  if (uri_.userinfo) {
    ...
    size_t info_len =
        username.size() + password.size() + sasl.size() + client.size() + 50;
...
__WFKafkaTask *
__WFKafkaTaskFactory::create_kafka_task(const std::string &url,
...
__WFKafkaTask *
__WFKafkaTaskFactory::create_kafka_task(const ParsedURI &uri, SSL_CTX *ssl_ctx,
                                        int retry_max,
                                        __kafka_callback_t callback) {
  auto *task = new __ComplexKafkaTask(retry_max, std::move(callback));
  task->set_ssl_ctx(ssl_ctx);
...
__WFKafkaTask *__WFKafkaTaskFactory::create_kafka_task(
    enum TransportType type, const char *host, unsigned short port,
    SSL_CTX *ssl_ctx, const std::string &info, int retry_max,
    __kafka_callback_t callback) {
  auto *task = new __ComplexKafkaTask(retry_max, std::move(callback));
  task->set_ssl_ctx(ssl_ctx);
