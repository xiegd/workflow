...
template <typename T> struct __NamedObjectList {
  __NamedObjectList(const std::string &str) : name(str) {
    INIT_LIST_HEAD(&this->head);
  }

  void push_back(T *node) { list_add_tail(&node->list, &this->head); }

  bool empty() const { return list_empty(&this->head); }
...
public:
  __NamedTimerMap() { root_.rb_node = NULL; }
...
public:
  __WFNamedTimerTask(time_t seconds, long nanoseconds, CommScheduler *scheduler,
                     timer_callback_t &&cb)
...
WFTimerTask *__NamedTimerMap::create(const std::string &name, time_t seconds,
                                     long nanoseconds, CommScheduler *scheduler,
                                     timer_callback_t &&cb) {
  auto *task =
      new __WFNamedTimerTask(seconds, nanoseconds, scheduler, std::move(cb));
  mutex_.lock();
...
WFTimerTask *WFTaskFactory::create_timer_task(const std::string &name,
  ...
  return __timer_map.create(name, seconds, nanoseconds,
                            WFGlobal::get_scheduler(), std::move(callback));
}
...
public:
  __NamedCounterMap() { root_.rb_node = NULL; }
} __counter_map;
...
public:
  ...
  virtual void count() { __counter_map.count(counters_, &node_); }
...
public:
  __NamedMailboxMap() { root_.rb_node = NULL; }
} __mailbox_map;
...
public:
  ...
  __WFNamedMailboxTask(mailbox_callback_t &&cb) : WFMailboxTask(std::move(cb)) {
    node_.task = this;
  }
  ...
  virtual void send(void *msg) { __mailbox_map.send(mailboxes_, &node_, msg); }
...
bool __NamedMailboxMap::send_max_locked(MailboxList *mailboxes, void *msg,
                                        size_t max,
                                        struct list_head *task_list) {
  if (max == (size_t)-1)
...
void __NamedMailboxMap::send(MailboxList *mailboxes,
                             struct __mailbox_node *node, void *msg) {
  bool erased;
...
void WFTaskFactory::send_by_name(const std::string &name, void *msg,
                                 size_t max) {
...
public:
  __NamedConditionalMap() { root_.rb_node = NULL; }
} __conditional_map;
...
public:
  ...
  __WFNamedConditional(SubTask *task) : WFConditional(task) {
    node_.cond = this;
  }
...
bool __NamedConditionalMap::signal_max_locked(ConditionalList *conds, void *msg,
                                              size_t max,
                                              struct list_head *cond_list) {
  if (max == (size_t)-1)
...
void __NamedConditionalMap::signal(const std::string &name, void *msg,
                                   size_t max) {
  LIST_HEAD(cond_list);
...
void __NamedConditionalMap::signal(ConditionalList *conds,
                                   struct __conditional_node *node, void *msg) {
  bool erased;
...
public:
  __NamedGuardMap() { root_.rb_node = NULL; }
} __guard_map;

class __WFNamedGuard : public WFConditional {
public:
  __WFNamedGuard(SubTask *task) : WFConditional(task) { node_.guard = this; }
