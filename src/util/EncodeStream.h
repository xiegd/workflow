...
public:
  ...
  void append_nocopy(const char *data) { append_nocopy(data, strlen(data)); }
  ...
  void append_copy(const char *data) { append_copy(data, strlen(data)); }
...
static inline EncodeStream &
operator<<(EncodeStream &stream, const std::pair<const char *, size_t> &data) {
