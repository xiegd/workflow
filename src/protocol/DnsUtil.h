...
class DnsUtil {
public:
  static int getaddrinfo(const DnsResponse *resp, unsigned short port,
                         struct addrinfo **res);
...
public:
  DnsResultCursor(const DnsResponse *resp) : parser(resp->get_parser()) {
  ...
  void reset_answer_cursor() { dns_answer_cursor_init(&cursor, parser); }

  void reset_authority_cursor() { dns_authority_cursor_init(&cursor, parser); }
