...
public:
  ...
  void set_config(const KafkaConfig &conf) { this->config = conf; }
  const KafkaConfig *get_config() const { return &this->config; }

  void set_cgroup(const KafkaCgroup &cgroup) { this->cgroup = cgroup; }
  KafkaCgroup *get_cgroup() { return &this->cgroup; }

  void set_broker(const KafkaBroker &broker) { this->broker = broker; }
  KafkaBroker *get_broker() { return &this->broker; }

  void set_meta_list(const KafkaMetaList &meta_list) {
    this->meta_list = meta_list;
  }
  KafkaMetaList *get_meta_list() { return &this->meta_list; }

  void set_toppar_list(const KafkaTopparList &toppar_list) {
    this->toppar_list = toppar_list;
  }
  KafkaTopparList *get_toppar_list() { return &this->toppar_list; }

  void set_broker_list(const KafkaBrokerList &broker_list) {
    this->broker_list = broker_list;
  }
  KafkaBrokerList *get_broker_list() { return &this->broker_list; }

  void set_sasl(kafka_sasl_t *sasl) { this->sasl = sasl; }

  void set_api(kafka_api_t *api) { this->api = api; }
...
protected:
  static int parse_message_set(void **buf, size_t *size, bool check_crcs,
  ...
  static int parse_record_batch(void **buf, size_t *size, bool check_crcs,
  ...
  static KafkaToppar *find_toppar_by_name(const std::string &topic,
                                          int partition,
