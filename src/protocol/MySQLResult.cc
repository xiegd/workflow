...
MySQLResultCursor::MySQLResultCursor() { this->init(); }
...
void MySQLResultCursor::fetch_result_set(
    const struct __mysql_result_set *result_set) {
  ...
  case MYSQL_PACKET_GET_RESULT:
    ...
    for (int i = 0; i < this->field_count; i++)
      this->fields[i] =
          new MySQLField(this->parser->buf, result_set->fields[i]);
    break;
...
bool MySQLResultCursor::fetch_row(
    std::unordered_map<std::string, MySQLCell> &row_map) {
  return this->fetch_row<std::unordered_map<std::string, MySQLCell>>(row_map);
}

bool MySQLResultCursor::fetch_row_nocopy(const void **data, size_t *len,
                                         int *data_type) {
  if (this->status != MYSQL_STATUS_GET_RESULT)
    return false;
