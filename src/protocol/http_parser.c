...
enum { HPS_START_LINE, HPS_HEADER_NAME, HPS_HEADER_VALUE, HPS_HEADER_COMPLETE };

enum { CPS_CHUNK_DATA, CPS_TRAILER_PART, CPS_CHUNK_COMPLETE };
...
static int __match_request_line(const char *method, const char *uri,
                                const char *version, http_parser_t *parser) {
...
static int __match_status_line(const char *version, const char *code,
                               const char *phrase, http_parser_t *parser) {
  if (strcmp(version, "HTTP/1.0") == 0 || strncmp(version, "HTTP/0", 6) == 0)
    parser->keep_alive = 0;
...
int http_parser_add_header(const void *name, size_t name_len, const void *value,
  ...
  if (__add_message_header(name, name_len, value, value_len, parser) >= 0) {
    __check_message_header((const char *)name, name_len, (const char *)value,
                           value_len, parser);
    return 0;
  }
