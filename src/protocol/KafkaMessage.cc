...
static int parse_bytes(void **buf, size_t *size, void **str, size_t *str_len);
...
static int compress_buf(KafkaBlock *block, int compress_type, void *env) {
  ...
  case Kafka_Lz4:
    ...
    lz4_r = LZ4F_compressUpdate(lz4_cctx, nblock.get_block(), nblock.get_len(),
                                block->get_block(), block->get_len(), NULL);
...
static int kafka_snappy_java_uncompress(const char *inbuf, size_t inlen,
                                        KafkaBlock *block) {
  char *obuf = NULL;
...
static int lz4_decompress(void *buf, size_t n, KafkaBlock *block) {
  ...
  while (in_off < inlen) {
    ...
    in_sz = inlen - in_off;
    r = LZ4F_decompress(dctx, out + out_off, &out_sz, inbuf + in_off, &in_sz,
                        NULL);
    if (LZ4F_isError(r)) {
      code = LZ4F_freeDecompressionContext(dctx);
      errno = EBADMSG;
...
static int zstd_decompress(void *buf, size_t n, KafkaBlock *block) {
  ...
  while (1) {
    ...
    ret = ZSTD_decompress(block->get_block(), out_bufsize, buf, n);
...
static int append_message_set(KafkaBlock *block, const KafkaRecord *record,
...
static int append_batch_record(KafkaBlock *block, const KafkaRecord *record,
...
static int append_record(KafkaBlock *block, const KafkaRecord *record,
                         int offset, int msg_version, const KafkaConfig &config,
                         int64_t first_timestamp, void *env, int cur_msg_size) {
  ...
  case 0:
  case 1:
    ret = append_message_set(block, record, offset, msg_version, config, env,
                             cur_msg_size);
    break;

  case 2:
    ret = append_batch_record(block, record, offset, config, first_timestamp,
                              env, cur_msg_size);
    break;
...
int KafkaMessage::parse_message_set(void **buf, size_t *size, bool check_crcs,
                                    int msg_vers, struct list_head *record_list,
  ...
  if (*size > 0) {
    return parse_message_set(buf, size, check_crcs, msg_vers, record_list,
                             uncompressed, toppar);
  }
...
static int parse_varint_bytes(void **buf, size_t *size, void **str,
                              size_t *str_len) {
  int64_t len = 0;
...
int KafkaMessage::parse_record_batch(void **buf, size_t *size, bool check_crcs,
  ...
  if (hdr.attributes & 7) {
    if (uncompress_buf(*buf, hdr.length - 61 + 12, &block, hdr.attributes & 7) <
        0)
      return -1;
...
int KafkaMessage::parse_records(void **buf, size_t *size, bool check_crcs,
  ...
  while (msg_size > 16) {
    ...
    case 0:
    case 1:
      ret = parse_message_set(buf, &msg_size, check_crcs, magic, record_list,
                              uncompressed, toppar);
      break;

    case 2:
      ret = parse_record_batch(buf, &msg_size, check_crcs, record_list,
                               uncompressed, toppar);
      break;
...
KafkaMessage::KafkaMessage() {
  static struct Crc32cInitializer {
    Crc32cInitializer() { crc32c_global_init(); }
  } initializer;
...
int KafkaMessage::encode_message(int api_type, struct iovec vectors[],
                                 int max) {
...
static int kafka_get_api_version(const kafka_api_t *api,
                                 const KafkaConfig &conf, int api_type,
                                 int max_ver, int message_version) {
  int min_ver = 0;
...
int KafkaMessage::encode_head() {
  if (this->api_type == Kafka_ApiVersions)
    ...
    this->api_version =
        kafka_get_api_version(this->api, this->config, this->api_type, max_ver,
...
static int kafka_compress_prepare(int compress_type, void **env,
  ...
  case Kafka_Lz4:
    ...
    lz4_r = LZ4F_compressBegin(lz4_cctx, block->get_block(), block->get_len(),
                               &kPrefs);
    if (LZ4F_isError(lz4_r)) {
      LZ4F_freeCompressionContext(lz4_cctx);
      errno = EBADMSG;
...
static int kafka_compress_finish(int compress_type, void *env,
  ...
  case Kafka_Lz4:
    ...
    lz4_r =
        LZ4F_compressEnd(lz4_cctx, block.get_block(), block.get_len(), NULL);
    if (LZ4F_isError(lz4_r)) {
      LZ4F_freeCompressionContext(lz4_cctx);
      errno = EBADMSG;
      return -1;
...
KafkaRequest::KafkaRequest() {
  ...
  this->encode_func_map[Kafka_Metadata] =
      std::bind(&KafkaRequest::encode_metadata, this, _1, _2);
  this->encode_func_map[Kafka_Produce] =
      std::bind(&KafkaRequest::encode_produce, this, _1, _2);
  this->encode_func_map[Kafka_Fetch] =
      std::bind(&KafkaRequest::encode_fetch, this, _1, _2);
  this->encode_func_map[Kafka_FindCoordinator] =
      std::bind(&KafkaRequest::encode_findcoordinator, this, _1, _2);
  this->encode_func_map[Kafka_JoinGroup] =
      std::bind(&KafkaRequest::encode_joingroup, this, _1, _2);
  this->encode_func_map[Kafka_SyncGroup] =
      std::bind(&KafkaRequest::encode_syncgroup, this, _1, _2);
  this->encode_func_map[Kafka_Heartbeat] =
      std::bind(&KafkaRequest::encode_heartbeat, this, _1, _2);
  this->encode_func_map[Kafka_OffsetFetch] =
      std::bind(&KafkaRequest::encode_offsetfetch, this, _1, _2);
  this->encode_func_map[Kafka_OffsetCommit] =
      std::bind(&KafkaRequest::encode_offsetcommit, this, _1, _2);
  this->encode_func_map[Kafka_ListOffsets] =
      std::bind(&KafkaRequest::encode_listoffset, this, _1, _2);
  this->encode_func_map[Kafka_LeaveGroup] =
      std::bind(&KafkaRequest::encode_leavegroup, this, _1, _2);
  this->encode_func_map[Kafka_ApiVersions] =
      std::bind(&KafkaRequest::encode_apiversions, this, _1, _2);
  this->encode_func_map[Kafka_SaslHandshake] =
      std::bind(&KafkaRequest::encode_saslhandshake, this, _1, _2);
  this->encode_func_map[Kafka_SaslAuthenticate] =
      std::bind(&KafkaRequest::encode_saslauthenticate, this, _1, _2);
}
...
int KafkaRequest::encode_produce(struct iovec vectors[], int max) {
  ...
  while ((toppar = this->toppar_list.get_next()) != NULL) {
    ...
    void *recordset_size_ptr =
        (void *)((char *)header_block.get_block() + header_block.get_len() - 4);
    ...
    while ((record = toppar->get_record_next()) != NULL) {
      ...
      if (record->get_timestamp() == 0) {
        ...
        record->set_timestamp((ts.tv_sec * 1000000000 + ts.tv_nsec) / 1000 /
                              1000);
      }
      ...
      int ret = append_record(
          &record_block, record, batch_cnt, this->message_version, this->config,
          first_timestamp, this->compress_env, batch_length);
    ...
    if (this->message_version == 2) {
      if (this->config.get_compress_type() != Kafka_NoCompress) {
        ...
        if (kafka_compress_finish(this->config.get_compress_type(),
                                  this->compress_env, &this->serialized,
                                  &addon) < 0) {
          return -1;
        }
...
int KafkaRequest::encode_metadata(struct iovec vectors[], int max) {
  ...
  if (this->api_version >= 4) {
    append_bool(this->msgbuf, this->config.get_allow_auto_topic_creation());
  }
...
int KafkaRequest::encode_joingroup(struct iovec vectors[], int max) {
  ...
  list_for_each(pos, this->cgroup.get_group_protocol()) {
    ...
    append_bytes(this->msgbuf, kafka_cgroup_gen_metadata(this->meta_list));
  }
...
KafkaResponse::KafkaResponse() {
  using namespace std::placeholders;
  this->parse_func_map[Kafka_Metadata] =
      std::bind(&KafkaResponse::parse_metadata, this, _1, _2);
  this->parse_func_map[Kafka_Produce] =
      std::bind(&KafkaResponse::parse_produce, this, _1, _2);
  this->parse_func_map[Kafka_Fetch] =
      std::bind(&KafkaResponse::parse_fetch, this, _1, _2);
  this->parse_func_map[Kafka_FindCoordinator] =
      std::bind(&KafkaResponse::parse_findcoordinator, this, _1, _2);
  this->parse_func_map[Kafka_JoinGroup] =
      std::bind(&KafkaResponse::parse_joingroup, this, _1, _2);
  this->parse_func_map[Kafka_SyncGroup] =
      std::bind(&KafkaResponse::parse_syncgroup, this, _1, _2);
  this->parse_func_map[Kafka_Heartbeat] =
      std::bind(&KafkaResponse::parse_heartbeat, this, _1, _2);
  this->parse_func_map[Kafka_OffsetFetch] =
      std::bind(&KafkaResponse::parse_offsetfetch, this, _1, _2);
  this->parse_func_map[Kafka_OffsetCommit] =
      std::bind(&KafkaResponse::parse_offsetcommit, this, _1, _2);
  this->parse_func_map[Kafka_ListOffsets] =
      std::bind(&KafkaResponse::parse_listoffset, this, _1, _2);
  this->parse_func_map[Kafka_LeaveGroup] =
      std::bind(&KafkaResponse::parse_leavegroup, this, _1, _2);
  this->parse_func_map[Kafka_ApiVersions] =
      std::bind(&KafkaResponse::parse_apiversions, this, _1, _2);
  this->parse_func_map[Kafka_SaslHandshake] =
      std::bind(&KafkaResponse::parse_saslhandshake, this, _1, _2);
  this->parse_func_map[Kafka_SaslAuthenticate] =
      std::bind(&KafkaResponse::parse_saslauthenticate, this, _1, _2);
}
...
static int kafka_meta_parse_broker(void **buf, size_t *size, int api_version,
...
static int kafka_meta_parse_partition(void **buf, size_t *size, KafkaMeta *meta,
                                      KafkaBrokerList *broker_list) {
  int32_t leader_id;
...
static KafkaMeta *find_meta_by_name(const std::string &topic,
...
static int kafka_meta_parse_topic(void **buf, size_t *size, int api_version,
                                  KafkaMetaList *meta_list,
                                  KafkaBrokerList *broker_list) {
  KafkaMetaList lst;
  int32_t topic_cnt;
...
KafkaToppar *KafkaMessage::find_toppar_by_name(const std::string &topic,
                                               int partition,
...
int KafkaResponse::parse_produce(void **buf, size_t *size) {
  ...
  for (int32_t topic_idx = 0; topic_idx < topic_cnt; ++topic_idx) {
    ...
    for (int32_t i = 0; i < partition_cnt; ++i) {
      ...
      KafkaToppar *toppar =
          find_toppar_by_name(topic_name, partition, &this->toppar_list);
      if (!toppar)
        return -1;
...
int KafkaResponse::parse_syncgroup(void **buf, size_t *size) {
  ...
  if (!member_assignment.empty()) {
    CHECK_RET(kafka_parse_member_assignment(
        member_assignment.c_str(), member_assignment.size(), &this->cgroup));
  }
...
int KafkaResponse::parse_offsetfetch(void **buf, size_t *size) {
  ...
  for (int32_t topic_idx = 0; topic_idx < topic_cnt; ++topic_idx) {
    ...
    for (int32_t i = 0; i < partition_cnt; ++i) {
      CHECK_RET(parse_i32(buf, size, &partition));
      KafkaToppar *toppar = find_toppar_by_name(
          topic_name, partition, this->cgroup.get_assigned_toppar_list());
      if (!toppar)
        return -1;
...
int KafkaResponse::parse_saslauthenticate(void **buf, size_t *size) {
  ...
  errno = 0;
  if (this->config.get_raw_ptr()->recv(auth_bytes.c_str(), auth_bytes.size(),
...
int KafkaResponse::append(const void *buf, size_t *size) {
  ...
  if (this->api_type == Kafka_SaslHandshake) {
    ...
    if (strncasecmp(this->config.get_sasl_mech(), "SCRAM", 5) == 0) {
      this->clear_buf();
      if (this->sasl->scram.state != KAFKA_SASL_SCRAM_STATE_CLIENT_FINISHED)
        return this->handle_sasl_continue();
      else
        this->sasl->status = 1;
    }
