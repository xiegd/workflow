...
static kafka_api_version_t kafka_api_version_0_9_0[] = {
    {Kafka_Produce, 0, 1},         {Kafka_Fetch, 0, 1},
    {Kafka_ListOffsets, 0, 0},     {Kafka_Metadata, 0, 0},
    {Kafka_OffsetCommit, 0, 2},    {Kafka_OffsetFetch, 0, 1},
    {Kafka_FindCoordinator, 0, 0}, {Kafka_JoinGroup, 0, 0},
    {Kafka_Heartbeat, 0, 0},       {Kafka_LeaveGroup, 0, 0},
    {Kafka_SyncGroup, 0, 0},       {Kafka_DescribeGroups, 0, 0},
    {Kafka_ListGroups, 0, 0}};

static kafka_api_version_t kafka_api_version_0_8_2[] = {
    {Kafka_Produce, 0, 0},        {Kafka_Fetch, 0, 0},
    {Kafka_ListOffsets, 0, 0},    {Kafka_Metadata, 0, 0},
    {Kafka_OffsetCommit, 0, 1},   {Kafka_OffsetFetch, 0, 1},
    {Kafka_FindCoordinator, 0, 0}};

static kafka_api_version_t kafka_api_version_0_8_1[] = {
    {Kafka_Produce, 0, 0},      {Kafka_Fetch, 0, 0},
    {Kafka_ListOffsets, 0, 0},  {Kafka_Metadata, 0, 0},
    {Kafka_OffsetCommit, 0, 1}, {Kafka_OffsetFetch, 0, 0}};
...
int kafka_api_version_is_queryable(const char *broker_version,
                                   kafka_api_version_t **api, size_t *api_cnt) {
...
static int kafka_api_version_check(const kafka_api_version_t *apis,
  ...
  api = bsearch(match, apis, api_cnt, sizeof(*apis), kafka_api_version_key_cmp);
  if (!api)
    return 0;
...
unsigned kafka_get_features(kafka_api_version_t *api, size_t api_cnt) {
  ...
  for (i = 0; kafka_feature_map[i].feature != 0; i++) {
    fails = 0;
    for (match = &kafka_feature_map[i].depends[0]; match->api_key != -1;
         match++) {
...
int kafka_broker_get_api_version(const kafka_api_t *api, int api_key,
  ...
  retp = bsearch(&sk, api->api, api->elements, sizeof(*api->api),
...
void kafka_parser_deinit(kafka_parser_t *parser) { free(parser->msgbuf); }
...
void kafka_api_deinit(kafka_api_t *api) { free(api->api); }
...
int kafka_record_header_set_kv(const void *key, size_t key_len, const void *val,
                               size_t val_len, kafka_record_header_t *header) {
  void *k = malloc(key_len);
  void *v = malloc(val_len);
...
static void scram_build_client_final_message_wo_proof(
  ...
  if (out->iov_base) {
    snprintf((char *)out->iov_base, out->iov_len + 1, "c=%s,r=%.*s%.*s", attr_c,
             (int)scram->cnonce.iov_len, (char *)scram->cnonce.iov_base,
             (int)snonce->iov_len, (char *)snonce->iov_base);
  }
}

static int scram_build_client_final_message(
    kafka_scram_t *scram, int itcnt, const struct iovec *salt,
    const struct iovec *server_first_msg, const struct iovec *server_nonce,
    struct iovec *out, const kafka_config_t *conf) {
  ...
  auth_message_iov.iov_len = scram->first_msg.iov_len + 1 +
                             server_first_msg->iov_len + 1 +
                             client_final_msg_wo_proof_iov.iov_len;
  ...
  if (auth_message_iov.iov_base) {
    snprintf(auth_message_iov.iov_base, auth_message_iov.iov_len + 1,
             "%.*s,%.*s,%.*s", (int)scram->first_msg.iov_len,
             (char *)scram->first_msg.iov_base, (int)server_first_msg->iov_len,
...
static int scram_handle_server_first_message(const char *buf, size_t len,
  ...
  if (scram_get_attr(&in, 'i', &out) == 0) {
    ...
    if ((const char *)out.iov_base != endptr && *endptr == '\0' &&
        itcnt <= 1000000) {
      ret = scram_build_client_final_message(&sasl->scram, itcnt, &salt, &in,
