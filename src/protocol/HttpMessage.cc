...
struct list_head *HttpMessage::combine_from(struct list_head *pos,
                                            size_t size) {
  size_t n = sizeof(struct HttpMessageBlock) + size;
  struct HttpMessageBlock *block = (struct HttpMessageBlock *)malloc(n);
...
HttpMessage::HttpMessage(HttpMessage &&msg) : ProtocolMessage(std::move(msg)) {
...
#define HTTP_100_RESP HTTP_100_STATUS_LINE CRLF CRLF
#define HTTP_400_RESP                                                          \
  HTTP_400_STATUS_LINE CRLF CONTENT_LENGTH_ZERO CRLF CONNECTION_CLOSE CRLF CRLF
#define HTTP_413_RESP                                                          \
  HTTP_413_STATUS_LINE CRLF CONTENT_LENGTH_ZERO CRLF CONNECTION_CLOSE CRLF CRLF
#define HTTP_417_RESP                                                          \
  HTTP_417_STATUS_LINE CRLF CONTENT_LENGTH_ZERO CRLF CONNECTION_CLOSE CRLF CRLF
