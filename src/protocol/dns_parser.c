...
static int __dns_parser_parse_a(struct __dns_record_entry **r,
                                uint16_t rdlength, dns_parser_t *parser) {
  const char **cur;
  struct __dns_record_entry *entry;
  size_t entry_size;
...
static int __dns_parser_parse_soa(struct __dns_record_entry **r,
  ...
  entry_size =
      sizeof(struct __dns_record_entry) + sizeof(struct dns_record_soa);
  entry = (struct __dns_record_entry *)malloc(entry_size);
  if (!entry)
    return -1;
...
static int __dns_parser_parse_srv(struct __dns_record_entry **r,
  ...
  *cur += 6;
  ...
  entry_size =
      sizeof(struct __dns_record_entry) + sizeof(struct dns_record_srv);
  entry = (struct __dns_record_entry *)malloc(entry_size);
  if (!entry)
    return -1;
...
int dns_parser_set_question(const char *name, uint16_t qtype, uint16_t qclass,
                            dns_parser_t *parser) {
  int ret;
...
int dns_record_cursor_find_cname(const char *name, const char **cname,
                                 dns_record_cursor_t *cursor) {
  struct __dns_record_entry *e;
...
int dns_add_soa_record(const char *name, uint16_t rclass, uint32_t ttl,
                       const char *mname, const char *rname, uint32_t serial,
                       int32_t refresh, int32_t retry, int32_t expire,
                       uint32_t minimum, struct list_head *list) {
  struct __dns_record_entry *entry;
  struct dns_record_soa *soa;
  size_t entry_size;
...
int dns_add_srv_record(const char *name, uint16_t rclass, uint32_t ttl,
                       ...
                       const char *target, struct list_head *list) {
  struct __dns_record_entry *entry;
  struct dns_record_srv *srv;
  size_t entry_size;
  char *pname, *ptarget;
