...
static bool operator<(const KafkaMetaSubscriber &s1,
                      const KafkaMetaSubscriber &s2) {
  ...
  for (auto &subscriber : *subscribers) {
    ...
    for (int i = 0; i < (int)subscriber.get_member()->size(); i++) {
      ...
      for (int j = start; j < length + start; ++j) {
        ...
        list_add_tail(&toppar->list,
                      &subscriber.get_member()->at(i)->assigned_toppar_list);
      }
    }
  }
    ...
    for (int partition = 0; partition < partition_elements; ++partition) {
      ...
      for (; i < member_elements; i++) {
        ...
        list_for_each(pos, &members[next + i]->toppar_list) {
          ...
          if (strcmp(subscriber.get_meta()->get_topic(), toppar->get_topic()) ==
              0) {
            flag = true;
            break;
          }
...
void KafkaCgroup::add_subscriber(
    KafkaMetaList *meta_list, std::vector<KafkaMetaSubscriber> *subscribers) {
  meta_list->rewind();
  KafkaMeta *meta;
