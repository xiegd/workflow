...
RedisValue::~RedisValue() { free_data(); }

RedisMessage::RedisMessage()
    : parser_(new redis_parser_t), stream_(new EncodeStream), cur_size_(0),
      asking_(false) {
...
void RedisRequest::set_request(const std::string &command,
  ...
  for (size_t i = 0; i < n; i++) {
    redis_reply_set_string(user_request_[i].c_str(), user_request_[i].size(),
                           reply->element[i]);
  }
}
