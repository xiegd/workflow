...
int MySQLMessage::encode(struct iovec vectors[], int max) {
  ...
  do {
    length = (nleft >= MYSQL_PAYLOAD_MAX ? MYSQL_PAYLOAD_MAX : (uint32_t)nleft);
    head = heads_[seqid];
...
int MySQLHandshakeResponse::decode_packet(const unsigned char *buf,
                                          size_t buflen) {
...
int MySQLSSLRequest::encode(struct iovec vectors[], int max) {
  ...
  int4store(pos, MYSQL_CAPFLAG_CLIENT_SSL | MYSQL_CAPFLAG_CLIENT_PROTOCOL_41 |
