...
static inline void floatstore(unsigned char *V, float M) { float4store(V, M); }
...
static inline void shortstore(unsigned char *T, int16_t V) { int2store(T, V); }

static inline void longstore(unsigned char *T, int32_t V) { int4store(T, V); }
...
static inline void int3store(unsigned char *T, uint32_t A) { memcpy(T, &A, 3); }
