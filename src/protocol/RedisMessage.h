...
inline RedisValue::RedisValue() : type_(REDIS_REPLY_TYPE_NIL), data_(NULL) {}

inline RedisValue::RedisValue(const RedisValue &copy)
    : type_(REDIS_REPLY_TYPE_NIL), data_(NULL) {
  this->operator=(copy);
}
...
inline bool RedisValue::is_ok() const {
  return type_ != REDIS_REPLY_TYPE_ERROR;
}
inline bool RedisValue::is_error() const {
  return type_ == REDIS_REPLY_TYPE_ERROR;
}
inline bool RedisValue::is_nil() const { return type_ == REDIS_REPLY_TYPE_NIL; }
inline bool RedisValue::is_int() const {
  return type_ == REDIS_REPLY_TYPE_INTEGER;
}
inline bool RedisValue::is_array() const {
  return type_ == REDIS_REPLY_TYPE_ARRAY;
}
...
inline std::string RedisValue::string_value() const {
  if (type_ == REDIS_REPLY_TYPE_STRING || type_ == REDIS_REPLY_TYPE_STATUS ||
...
inline void RedisValue::clear() { set_nil(); }
...
inline redis_reply_t *RedisResponse::result_ptr() { return &parser_->reply; }
