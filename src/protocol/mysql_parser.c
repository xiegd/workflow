...
static int parse_base_packet(const void *buf, size_t len,
                             mysql_parser_t *parser);

static int parse_error_packet(const void *buf, size_t len,
                              mysql_parser_t *parser);

static int parse_ok_packet(const void *buf, size_t len, mysql_parser_t *parser);

static int parse_eof_packet(const void *buf, size_t len,
                            mysql_parser_t *parser);

static int parse_field_eof_packet(const void *buf, size_t len,
                                  mysql_parser_t *parser);

static int parse_field_count(const void *buf, size_t len,
                             mysql_parser_t *parser);

static int parse_column_def_packet(const void *buf, size_t len,
                                   mysql_parser_t *parser);

static int parse_local_inline(const void *buf, size_t len,
                              mysql_parser_t *parser);

static int parse_row_packet(const void *buf, size_t len,
                            mysql_parser_t *parser);
...
void mysql_parser_get_net_state(const char **net_state_str,
                                size_t *net_state_len, mysql_parser_t *parser) {
  *net_state_str = (const char *)parser->buf + parser->net_state_offset;
  *net_state_len = MYSQL_STATE_LENGTH;
}

void mysql_parser_get_err_msg(const char **err_msg_str, size_t *err_msg_len,
                              mysql_parser_t *parser) {
...
static int parse_ok_packet(const void *buf, size_t len,
                           mysql_parser_t *parser) {
  const unsigned char *p = (const unsigned char *)buf + parser->offset;
  const unsigned char *buf_end = (const unsigned char *)buf + len;
  ...
  result_set =
      (struct __mysql_result_set *)malloc(sizeof(struct __mysql_result_set));
  if (result_set == NULL)
    return -1;
...
static int parse_field_count(const void *buf, size_t len,
  ...
  if (field_count) {
    ...
    result_set->fields =
        (mysql_field_t **)calloc(field_count, sizeof(mysql_field_t *));
...
// 2:charsetnr|4:length|1:type|2:flags|1:decimals|1:0x00|1:0x00|n:str(if
// COM_FIELD_LIST)
static int parse_column_def_packet(const void *buf, size_t len,
                                   mysql_parser_t *parser) {
  const unsigned char *p = (const unsigned char *)buf + parser->offset;
