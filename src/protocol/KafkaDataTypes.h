...
template <class T> class KafkaList {
public:
  KafkaList() {
  ...
  struct list_head *get_tail() {
    return this->t_list->prev;
  }
  ...
  T *get_entry(struct list_head *pos) { return list_entry(pos, T, list); }

  void rewind() { this->curpos = this->t_list; }
...
template <class T> class KafkaMap {
public:
  KafkaMap() {
...
public:
  ...
  void set_produce_msgset_cnt(int cnt) { this->ptr->produce_msgset_cnt = cnt; }
  int get_produce_msgset_cnt() const { return this->ptr->produce_msgset_cnt; }
  ...
  void set_fetch_msg_max_bytes(int bytes) {
    this->ptr->fetch_msg_max_bytes = bytes;
  }
  ...
  int get_api_version_request() const { return this->ptr->api_version_request; }
  ...
  const char *get_broker_version() const { return this->ptr->broker_version; }
  ...
  bool get_check_crcs() const { return this->ptr->check_crcs != 0; }
  void set_check_crcs(bool check_crcs) { this->ptr->check_crcs = check_crcs; }

  int get_offset_store() const { return this->ptr->offset_store; }
  void set_offset_store(int offset_store) {
    this->ptr->offset_store = offset_store;
  }

  const char *get_rack_id() const { return this->ptr->rack_id; }
  ...
  const char *get_sasl_mech() const { return this->ptr->mechanisms; }
  ...
  const char *get_sasl_username() const { return this->ptr->username; }
  bool set_sasl_username(const char *username) {
    return kafka_sasl_set_username(username, this->ptr) == 0;
  }

  const char *get_sasl_password() const { return this->ptr->password; }
...
public:
  ...
  bool add_header_pair(const void *key, size_t key_len, const void *val,
                       size_t val_len);
...
public:
  ...
  struct list_head *get_header_list() const {
    return &this->ptr->header_list;
  }
...
public:
  bool set_topic_partition(const std::string &topic, int partition) {
    return kafka_topic_partition_set_tp(topic.c_str(), partition, this->ptr) ==
           0;
  }
  ...
  void set_high_watermark(long long offset) const {
    this->ptr->high_watermark = offset;
  }

  long long get_low_watermark() const { return this->ptr->low_watermark; }
  void set_low_watermark(long long offset) {
    this->ptr->low_watermark = offset;
  }
...
public:
  ...
  struct list_head *get_record() {
    return &this->ptr->record_list;
  }
  ...
  void record_rewind() { this->curpos = &this->ptr->record_list; }
  ...
  void save_record_startpos() { this->startpos = this->curpos; }

  void save_record_endpos() { this->endpos = this->curpos->next; }

  bool record_reach_end() { return this->endpos == &this->ptr->record_list; }

  void record_rollback() { this->curpos = this->curpos->prev; }

  KafkaRecord *get_tail_record() {
    if (&this->ptr->record_list != this->ptr->record_list.prev) {
      return (KafkaRecord *)list_entry(this->ptr->record_list.prev, KafkaRecord,
                                       list);
    } else {
      return NULL;
    }
...
class KafkaBroker {
public:
  const char *get_host() const { return this->ptr->host; }

  int get_port() const { return this->ptr->port; }
  ...
  int get_error() { return this->ptr->error; }
...
public:
  void set_meta(KafkaMeta *meta) { this->meta = meta; }

  const KafkaMeta *get_meta() const { return this->meta; }
...
public:
  ...
  static int kafka_range_assignor(kafka_member_t **members, int member_elements,
                                  void *meta_topic);

  static int kafka_roundrobin_assignor(kafka_member_t **members,
                                       int member_elements, void *meta_topic);
...
public:
  ...
  void backup(size_t n) { this->buf_size -= n; }
  ...
  KafkaBlock *get_block_tail() { return this->block_list.get_tail_entry(); }
  ...
  KafkaBlock *get_block_next() { return this->block_list.get_next(); }
  ...
  size_t get_size() const { return this->buf_size; }
...
public:
  KafkaSnappySink(KafkaBuffer *buffer) { this->buffer = buffer; }

  virtual void Append(const char *bytes, size_t n) {
    this->buffer->append(bytes, n);
  }

  size_t size() const { return this->buffer->get_size(); }

  KafkaBuffer *get_buffer() const { return buffer; }
...
public:
  ...
  virtual size_t Available() const { return this->buf_size - this->pos; }
  ...
  virtual void Skip(size_t n) { this->pos += this->buffer->seek(n); }
