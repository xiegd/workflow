...
class DnsInput {
public:
  DnsInput() : port_(0), numeric_host_(false), family_(AF_UNSPEC) {}

  DnsInput(const std::string &host, unsigned short port, bool numeric_host,
           int family)
      : host_(host), port_(port), numeric_host_(numeric_host), family_(family) {
  }
  ...
  void reset(const std::string &host, unsigned short port, bool numeric_host,
             int family) {
    host_.assign(host);
    port_ = port;
...
class DnsOutput {
public:
  DnsOutput() : error_(0), addrinfo_(NULL) {}
...
void DnsRoutine::run(const DnsInput *in, DnsOutput *out) {
  ...
  out->error_ =
      getaddrinfo(in->host_.c_str(), port_str, &hints, &out->addrinfo_);
  if (out->error_ == 0)
...
static int __readaddrinfo(const char *path, const char *name,
                          unsigned short port, const struct addrinfo *hints,
                          struct addrinfo **res) {
  char port_str[PORT_STR_MAX + 1];
  size_t bufsize = 0;
  char *line = NULL;
...
static ThreadDnsTask *__create_thread_dns_task(const std::string &host,
                                               unsigned short port, int family,
                                               thread_dns_callback_t callback) {
  auto *task = WFThreadTaskFactory<DnsInput, DnsOutput>::create_thread_task(
      WFGlobal::get_dns_queue(), WFGlobal::get_dns_executor(), DnsRoutine::run,
      std::move(callback));
...
static std::string __get_cache_host(const std::string &hostname, int family) {
  char c;
...
void WFResolverTask::request_dns() {
  ...
  if (client) {
    ...
    if (family == AF_INET || family == AF_INET6) {
      auto &&cb = std::bind(&WFResolverTask::dns_single_callback, this,
                            std::placeholders::_1);
      ...
      auto &&cb = std::bind(&WFResolverTask::dns_parallel_callback, this,
                            std::placeholders::_1);
    ...
    auto &&cb = std::bind(&WFResolverTask::thread_dns_callback, this,
                          std::placeholders::_1);
    dns_task = __create_thread_dns_task(host_, port_, ep_params_.address_family,
                                        std::move(cb));
...
void WFResolverTask::dns_callback_internal(void *thrd_dns_output,
  ...
  if (dns_error) {
    ...
    addr_handle =
        dns_cache->put(cache_host, port_, addrinfo, (unsigned int)ttl_default,
                       (unsigned int)ttl_min);
    if (route_manager->get(ns_params_.type, addrinfo, ns_params_.info,
                           &ep_params_, hostname, ns_params_.ssl_ctx,
                           this->result) < 0) {
