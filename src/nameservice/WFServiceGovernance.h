...
struct AddressParams {
  ...
  unsigned int dns_ttl_min; ///< in seconds, DNS TTL when network request fail
                             ...
                             * - After 30s following the server failure, upstream probe the server with
                             * some live client`s requests.
                             * - If the probes have been successful, the server is marked as a live one.
                             * - If max_fails is set to 1, it means server would out of upstream selection
                             * in 30 seconds when failed only once
                             */
  unsigned int max_fails; ///< [1, INT32_MAX] max_fails = 0 means max_fails = 1
  unsigned short
      weight; ///< [1, 65535] weight = 0 means weight = 1. only for main server
  int server_type; ///< 0 for main and 1 for backup
  int group_id; ///< -1 means no group. Backup without group will be backup for
                ///< any main
};
...
private:
  ...
  virtual void fuse_one_server(const EndpointAddress *addr) { this->nalives--; }
...
protected:
  ...
  std::unordered_map<std::string, std::vector<EndpointAddress *>> server_map;
