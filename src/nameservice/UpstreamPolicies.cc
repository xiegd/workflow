...
public:
  EndpointGroup(int group_id, UPSGroupPolicy *policy)
      : mutex(PTHREAD_MUTEX_INITIALIZER), gen(random()) {
    this->id = group_id;
    this->policy = policy;
    this->nalives = 0;
    this->weight = 0;
...
inline bool UPSGroupPolicy::is_alive(const EndpointAddress *addr) const {
  ...
  return (
      (params->group_id < 0 && addr->fail_count < addr->params->max_fails) ||
      (params->group_id >= 0 && params->group->nalives > 0));
}
...
void UPSGroupPolicy::add_server(const std::string &address,
  ...
  EndpointAddress *addr =
      new EndpointAddress(address, new UPSAddrParams(params, address));
 ...
 * 					 If addr is not available, get one from
 * addr->group.
 */
...
EndpointAddress *
UPSGroupPolicy::consistent_hash_with_group(unsigned int hash,
                                           WFNSTracing *tracing) {
  if (this->nalives == 0)
    return NULL;
...
void UPSGroupPolicy::hash_map_add_addr(EndpointAddress *addr) {
  ...
  if (params->server_type == 0) {
    ...
    for (int i = 0; i < VIRTUAL_GROUP_SIZE * params->weight; i++) {
      hash_value = std_hash(addr->address + "|v" + std::to_string(i) + "|n" +
                            std::to_string(ip_count));
...
EndpointAddress *
UPSWeightedRandomPolicy::another_strategy(const ParsedURI &uri,
                                          WFNSTracing *tracing) {
...
void UPSVNSWRRPolicy::init_virtual_nodes() {
  ...
  for (size_t i = start_pos; i < end_pos; i++) {
    ...
    std::vector<int>::iterator biggest = std::max_element(
        this->current_weight_vec.begin(), this->current_weight_vec.end());
    this->pre_generated_vec[i] =
        std::distance(this->current_weight_vec.begin(), biggest);
...
EndpointAddress *UPSManualPolicy::first_strategy(const ParsedURI &uri,
                                                 WFNSTracing *tracing) {
  unsigned int idx =
      this->manual_select(uri.path ? uri.path : "", uri.query ? uri.query : "",
                          uri.fragment ? uri.fragment : "");
...
EndpointAddress *UPSManualPolicy::another_strategy(const ParsedURI &uri,
                                                   WFNSTracing *tracing) {
  unsigned int hash_value =
      this->another_select(uri.path ? uri.path : "", uri.query ? uri.query : "",
                           uri.fragment ? uri.fragment : "");
  return this->consistent_hash_with_group(hash_value, tracing);
}
