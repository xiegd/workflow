...
using upstream_route_t = std::function<unsigned int(
...
public:
  UPSRoundRobinPolicy(bool try_another) : cur_idx(0) {
...
public:
  ...
  EndpointAddress *first_strategy(const ParsedURI &uri, WFNSTracing *tracing);
  EndpointAddress *another_strategy(const ParsedURI &uri, WFNSTracing *tracing);
