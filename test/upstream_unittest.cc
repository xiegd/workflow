...
WFHttpServer http_server1(std::bind(&__http_process, std::placeholders::_1,
                                    "server1"));
WFHttpServer http_server2(std::bind(&__http_process, std::placeholders::_1,
                                    "server2"));
WFHttpServer http_server3(std::bind(&__http_process, std::placeholders::_1,
                                    "server3"));
...
void register_upstream_hosts() {
  ...
  UpstreamManager::upstream_add_server("weighted.random", "127.0.0.1:8001",
                                       &address_params);
  address_params.weight = 1;
  UpstreamManager::upstream_add_server("weighted.random", "127.0.0.1:8002",
                                       &address_params);

  UpstreamManager::upstream_create_consistent_hash(
      "hash",
      [](const char *path, const char *query,
         const char *fragment) -> unsigned int {
  ...
  UpstreamManager::upstream_create_manual(
      "manual",
      [](const char *path, const char *query,
         const char *fragment) -> unsigned int { return 0; },
      true,
      [](const char *path, const char *query,
         const char *fragment) -> unsigned int {
  ...
  address_params.weight = 1000;
  UpstreamManager::upstream_add_server("test_tracing", "127.0.0.1:8001",
                                       &address_params);
  address_params.weight = 1;
  UpstreamManager::upstream_add_server("test_tracing", "127.0.0.1:8002",
                                       &address_params);
  address_params.weight = 1000;
  UpstreamManager::upstream_add_server("test_tracing", "127.0.0.1:8003",
                                       &address_params);
}
...
void basic_callback(WFHttpTask *task, std::string &message) {
  ...
  WFFacilities::WaitGroup *wait_group =
      (WFFacilities::WaitGroup *)task->user_data;
  wait_group->done();
}
...
TEST(upstream_unittest, BasicPolicy) {
  ...
  char url[4][30] = {"http://weighted.random", "http://manual", "http://hash",
                     "http://round.robin"};

  http_callback_t cb1 =
      std::bind(basic_callback, std::placeholders::_1, std::string("server1"));
  for (int i = 0; i < 2; i++) {
    task1 =
        WFTaskFactory::create_http_task(url[i], REDIRECT_MAX, RETRY_MAX, cb1);
    task1->user_data = &wait_group;
    task1->start();
  }

  http_callback_t cb2 =
      std::bind(basic_callback, std::placeholders::_1, std::string("server2"));

  task2 = WFTaskFactory::create_http_task(url[2], REDIRECT_MAX, RETRY_MAX, cb2);
  task2->user_data = &wait_group;
  task2->start();

  task1 = WFTaskFactory::create_http_task(url[3], REDIRECT_MAX, RETRY_MAX, cb1);
  task1->user_data = &wait_group;

  task2 = WFTaskFactory::create_http_task(url[3], REDIRECT_MAX, RETRY_MAX, cb2);
  task2->user_data = &wait_group;
...
TEST(upstream_unittest, EnableAndDisable) {
  ...
  WFHttpTask *task = WFTaskFactory::create_http_task(
      url, REDIRECT_MAX, RETRY_MAX, [&wait_group, &url](WFHttpTask *task) {
        int state = task->get_state();
        EXPECT_EQ(state, WFT_STATE_TASK_ERROR);
        EXPECT_EQ(task->get_error(), WFT_ERR_UPSTREAM_UNAVAILABLE);
        UpstreamManager::upstream_enable_server("weighted.random",
                                                "127.0.0.1:8001");
        auto *task2 = WFTaskFactory::create_http_task(
            url, REDIRECT_MAX, RETRY_MAX,
            std::bind(basic_callback, std::placeholders::_1,
                      std::string("server1")));
        task2->user_data = &wait_group;
...
TEST(upstream_unittest, FuseAndRecover) {
  ...
  fprintf(stderr, "server 1 stopped start %d tasks to fuse it\n", batch);
  ParallelWork *pwork =
      Workflow::create_parallel_work([](const ParallelWork *pwork) {
  ...
  WFTimerTask *timer =
      WFTaskFactory::create_timer_task(timeout, [](WFTimerTask *task) {
...
TEST(upstream_unittest, TryAnother) {
  ...
  WFHttpTask *task = WFTaskFactory::create_http_task(
      "http://manual", REDIRECT_MAX, RETRY_MAX, cb2);
  task->user_data = &wait_group;
  task->start();

  // this->cur_idx == 1. Will skip 8001 and try 8002.
  task = WFTaskFactory::create_http_task("http://round.robin", REDIRECT_MAX,
                                         RETRY_MAX, cb2);
  task->user_data = &wait_group;
  task->start();

  task = WFTaskFactory::create_http_task(
      "http://try_another", REDIRECT_MAX, RETRY_MAX,
...
TEST(upstream_unittest, Tracing) {
  ...
  // test first_strategy()
  WFHttpTask *task = WFTaskFactory::create_http_task(
      "http://weighted.random", REDIRECT_MAX, RETRY_MAX,
      std::bind(basic_callback, std::placeholders::_1, std::string("server2")));
  task->user_data = &wait_group;
  task->start();

  // test another_strategy()
  UpstreamManager::upstream_disable_server("test_tracing", "127.0.0.1:8003");
  WFHttpTask *task2 = WFTaskFactory::create_http_task(
      "http://test_tracing", REDIRECT_MAX, RETRY_MAX,
      std::bind(basic_callback, std::placeholders::_1, std::string("server2")));
  task2->user_data = &wait_group;
  task2->start();
...
TEST(upstream_unittest, RoundRobin) {
  ...
  WFHttpTask *task = WFTaskFactory::create_http_task(
      "http://round.robin", REDIRECT_MAX, RETRY_MAX,
      std::bind(basic_callback, std::placeholders::_1, std::string("server1")));
  task->user_data = &wait_group;
  task->start();
