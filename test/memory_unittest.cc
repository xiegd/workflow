...
TEST(memory_unittest, dismiss) {
  ...
  auto *http_task =
      WFTaskFactory::create_http_task("http://www.sogou.com", 0, 0, nullptr);
  tasks.push_back(http_task);

  auto *redis_task = WFTaskFactory::create_redis_task(
      "redis://username:password@127.0.0.1:6676/1", 0, nullptr);
  tasks.push_back(redis_task);

  auto *mysql_task = WFTaskFactory::create_mysql_task(
      "mysql://username:password@127.0.0.1:8899/db", 0, nullptr);
  ...
  auto *thread_task = WFThreadTaskFactory<int, int>::create_thread_task(
      "", [](int *, int *) {}, nullptr);
  tasks.push_back(thread_task);

  auto *graph_task = WFTaskFactory::create_graph_task(nullptr);
  auto &node_a = graph_task->create_graph_node(
      WFTaskFactory::create_timer_task(0, nullptr));
  auto &node_b = graph_task->create_graph_node(
      WFTaskFactory::create_timer_task(0, nullptr));
  node_a-- > --node_b;
