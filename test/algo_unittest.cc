...
TEST(algo_unittest, sort) {
  ...
  bool done = false;
  auto *task = WFAlgoTaskFactory::create_sort_task(
      "sort", arr, arr + n, [&mutex, &cond, &done](WFSortTask<int> *task) {
...
TEST(algo_unittest, parallel_sort) {
  ...
  std::condition_variable cond;
  bool done = false;
  auto *task = WFAlgoTaskFactory::create_psort_task(
      "psort", arr, arr + n, [&mutex, &cond, &done](WFSortTask<int> *task) {
        int *first = task->get_input()->first;
        int *last = task->get_input()->last;
        __arr_check(first, last - first);
        mutex.lock();
