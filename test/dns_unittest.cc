...
TEST(dns_unittest, WFDnsTaskCreate2) {
  ...
  auto *task = WFTaskFactory::create_dns_task(
      url, 0, [&done](WFDnsTask *task) { done.set_value(); });
  task->start();
  done.get_future().get();
}
...
TEST(dns_unittest, WFDnsTask) {
  ...
  auto *task = WFTaskFactory::create_dns_task(
      url, RETRY_MAX, [&done, req_id](WFDnsTask *task) {
...
TEST(dns_unittest, WFDnsClient) {
  ...
  auto *task =
      client.create_dns_task("www.sogou.com", [&done, req_id](WFDnsTask *task) {
        int state = task->get_state();
