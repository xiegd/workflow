...
TEST(facilities_unittest, request) {
  ...
  auto res =
      WFFacilities::request<protocol::HttpRequest, protocol::HttpResponse>(
          TT_TCP, "http://github.com", std::move(req), 0);
  // EXPECT_EQ(res.task_state, WFT_STATE_SUCCESS);
  if (res.task_state == WFT_STATE_SUCCESS) {
    auto code = atoi(res.resp.get_status_code());
    EXPECT_TRUE(code == HttpStatusOK || code == HttpStatusMovedPermanently ||
...
TEST(facilities_unittest, async_request) {
  ...
  auto res =
      WFFacilities::request<protocol::HttpRequest, protocol::HttpResponse>(
          TT_TCP_SSL, "https://github.com", std::move(req), 0);
  // EXPECT_EQ(res.task_state, WFT_STATE_SUCCESS);
  if (res.task_state == WFT_STATE_SUCCESS) {
    auto code = atoi(res.resp.get_status_code());
    EXPECT_TRUE(code == HttpStatusOK || code == HttpStatusMovedPermanently ||
...
static inline void f(int i, WFFacilities::WaitGroup *wg) { wg->done(); }
